...
export default function Forge({ rawSubsList }: { rawSubsList: any[] }) {
  ...
  useEffect(() => {
    ...
    let currentForge: Record<string, ForgeItem> = {};
    try { currentForge = JSON.parse(localStorage.getItem('cf_forge_v1') || '{}'); } catch { currentForge = {}; }
    ...
    try { if (updated) localStorage.setItem('cf_forge_v1', JSON.stringify(currentForge)); } catch { console.warn('[Forge] Could not persist to localStorage.'); }
  ...
  const markFailedAgain = (pid: string) => {
    ...
    try { localStorage.setItem('cf_forge_v1', JSON.stringify(newForge)); } catch { console.warn('[Forge] Could not persist to localStorage.'); }
    setForge(newForge);
  };
