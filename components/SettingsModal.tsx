...
export default function SettingsModal({ onClose, onSave }: { onClose: () => void, onSave: (cfg: any) => void }) {
  ...
  const handleSave = () => {
    const cfg = {
      ...
      titans: [] as string[], // titans are managed in TitanTab
    };
    ...
    <div className="fixed inset-0 bg-black/85 backdrop-blur-sm flex justify-center items-center z-[1000] p-4">
      <div className="bg-[#1e2024] p-10 rounded-xl border border-[#30363d] w-full max-w-lg shadow-2xl">
        ...
        <div className="space-y-5 font-mono">
          ...
          <div className="text-[11px] font-mono text-[#555] border border-[#1a1a1a] rounded p-3 bg-[#0a0a0a]">
            💀 Titans are managed directly in the <span className="text-[#f85149]">TITAN</span> tab — add as many targets as you want there.
          </div>
        </div>
